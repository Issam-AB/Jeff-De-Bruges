...
// Use the predefined categories from the categories file
const getCategoryStructure = () => {
  const structure: { [key: string]: string[] } = {}
  PREDEFINED_CATEGORIES.forEach(category => {
    if (category.children && category.children.length > 0) {
      structure[category.name] = category.children.map(child => child.name)
    } else {
...
const CATEGORY_STRUCTURE = getCategoryStructure()
