...
import { Info, DollarSign, Flame, Star, Upload, X, Image as ImageIcon } from 'lucide-react'
...
export function EditProductForm({ product, onClose, onProductUpdated }: EditProductFormProps) {
  const defaultMainCategory = PREDEFINED_CATEGORIES[0]?.id || 'tous'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updateData = {
        ...
        isTopProduct: Boolean(formData.isTopProduct),
      }
    ...
    <div className="max-h-[90vh] overflow-y-auto">
      <form onSubmit={handleSubmit} className="space-y-4">
        <DialogHeader className="pb-3 border-b border-gray-700">
          ...
          <p className="text-gray-400 text-xs">Update product information and settings</p>
        </DialogHeader>
